...
class KeysControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create key" do
    assert_difference("Key.count") do
      post keys_url, params: { key: { code: @key.code, description: @key.description, status: @key.status, table_id: @key.table_id } }
    end
  ...
  test "should update key" do
    patch key_url(@key), params: { key: { code: @key.code, description: @key.description, status: @key.status, table_id: @key.table_id } }
